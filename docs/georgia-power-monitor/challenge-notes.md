...
- **Deploy host (desktop vs. xps-agent) is not yet decided** — now a pure load/capacity call since the egress-colocation constraint was removed; needs Preston's or the build pipeline's decision at deploy time based on current load.
