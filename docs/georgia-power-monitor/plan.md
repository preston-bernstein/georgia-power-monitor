...
A daily systemd-timer oneshot CLI (`gp-monitor poll`), structurally identical to `algo-macro-monitor`
...
in `home-infra` today, so REST is the only viable path, not a stopgap).
...
gp-monitor poll  (oneshot CLI, src/gp_monitor/cli.py)
        ...
        │     POST /api/states/<entity_id> per entity to 10.0.0.5:8123, Bearer token
...
repo simpler than `algo-macro-monitor` (which needs local SQLite because it correlates against a
...
retry state across scheduled runs — mirrors the shape fashion-monitor's Poshmark login circuit
...
```
POST http://10.0.0.5:8123/api/states/sensor.georgia_power_usage_kwh
...
- `src/gp_monitor/breaker.py` — **new**: the JSON circuit-breaker file (see Data model), same
  5-consecutive-failure trip shape as fashion-monitor's `poshmark/login-circuit-breaker.ts`.
...
- `src/gp_monitor/log.py`, `src/gp_monitor/config.py` — thin wrappers around `fleet_logging`, copied
  verbatim from `algo-macro-monitor/src/macro_monitor/{log,config}.py`'s pattern (stderr channel,
...
- `pyproject.toml` — setuptools, `src/` layout, `southern-company-api` + `scraper-commons` (`rate`,
  ...
  git-pin convention as `algo-macro-monitor/pyproject.toml`.
- `systemd/gp-monitor-poll.service` + `.timer` — `Type=oneshot`, `EnvironmentFile=/home/gp-monitor/app/.env`
  ...
  `algo-macro-monitor/systemd/macro-monitor-collect.service` (`ProtectSystem=strict`, `NoNewPrivileges`,
  etc.), `OnCalendar` daily (see Risk areas / poll-interval justification below) (FR-5/FR-13). Deploy
  host (desktop vs. xps-agent) is a pure load/capacity decision — there is no proxy-colocation
...
- `.env.example` — `GP_USERNAME=`, `GP_PASSWORD=`, `HA_LONG_LIVED_TOKEN=`, documented, no real values
  (FR-14/FR-11).
- `scripts/deploy.sh` — mirrors `algo-macro-monitor/scripts/deploy.sh`: verify service user exists,
...
- **HA REST API with a long-lived token, not MQTT discovery** — confirmed no MQTT broker exists
  anywhere in `home-infra` (`docker-compose.yml`, ADR list, and a code search all came up empty).
  REST is not a stopgap here; it is what the fleet actually has today, per FR-7's own stated fallback.
- **`httpx`** for the HA REST client — already a dependency pattern in `algo-macro-monitor`
...
- **`fleet_logging`** (git-pinned) + the thin `log.py`/`config.py` wrapper pattern — copied from
  `algo-macro-monitor`, not reimplemented, per the explicit FR-10 constraint.
...
- **Plain JSON circuit-breaker file**, not `scraper-commons`' `admission` module — `admission` solves
  ...
  failures then require a human"; fashion-monitor already established this exact plain-JSON pattern
...
2. **Egress isolation is deliberately dropped from this integration, not deferred by oversight.**
   ...
   has a documented history (a comment in `home-infra`'s own compose file) of silently breaking
   existing `network_mode: container:gluetun-scraper` attachments — currently used by
   fashion-monitor's scraper/poshmark services (desktop) and arr-stack's Shelfarr (xps-agent) — a
