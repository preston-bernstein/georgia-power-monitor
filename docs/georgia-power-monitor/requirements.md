...
- Home Assistant instance at 10.0.0.5:8123 — the downstream system that receives and renders the published data; not managed/SSH-able by Preston's fleet.
- The managed deploy host (desktop 10.0.0.243 or xps-agent 10.0.0.244, selected per current load distribution) — runs the scheduled poll job.
...
7. The system shall publish retrieved usage and billing data to Home Assistant at 10.0.0.5:8123 over its REST API using a long-lived access token (no MQTT broker exists in home-infra as of this build, so REST is the integration path, not a placeholder pending a future decision).
...
- Deploy host selection must account for current load: xps-agent already runs arr-stack + fashion-monitor, so host choice is based on which managed host (desktop or xps-agent) has verified LAN reachability to 10.0.0.5 and spare capacity, decided during the plan phase.
...
- Home Assistant at 10.0.0.5:8123 is not one of Preston's managed/SSH-able hosts (unlike desktop, NAS, xps-agent, pihole) — all integration must happen over the LAN network via the HA REST API, never via local install or file-level access to the HA host.
...
- Deploy target is one of the existing managed hosts (desktop or xps-agent) — no new host is provisioned for this feature.
...
- Local installation onto the Home Assistant host itself (10.0.0.5 is not managed/SSH-able).
...
2. Given a successful poll cycle, the corresponding usage and billing values appear as distinct entities in Home Assistant at 10.0.0.5:8123 within one poll interval of the scrape completing.
