...
7. The system shall publish retrieved usage and billing data to Home Assistant at <ha-host>:8123 over its REST API using a long-lived access token (no MQTT broker exists in internal-infra as of this build, so REST is the integration path, not a placeholder pending a future decision).
