...
#
# Mirrors internal-monitor-service/scripts/deploy.sh's structure and service-user convention, adapted
# for gp-monitor: this script provisions the service user itself (internal-monitor-service's assumes the user is
# already provisioned) and installs into a dedicated venv dir rather than an in-app .venv. Intended
# to be run ON the target deploy host (desktop or vmhost.example.internal) from a checked-out clone of this repo
...
# 1a. Group membership for the shared node-exporter textfile-collector directory (internal-infra
...
#     fleet service (e.g. internal-monitor-service). Only wire this in if that group actually exists on this
