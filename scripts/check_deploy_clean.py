...
rsynced to the desktop by a routine internal-infra deploy and killed all Loki log shipping.
internal-infra's deploy scripts gained a repo-side gate for this
(tools/config-drift/preflight.sh + drift_audit.py's --check-shipped-clean, see internal-infra
ADR 0019 and PR #104) -- this repo's deploy.sh had no equivalent at all.

WHY A SEPARATE SCRIPT INSTEAD OF REUSING internal-infra's drift_audit.py DIRECTLY. That tool's
...
matching manifest entry -- could sys.exit(2) internal-infra's OWN, unrelated deploys the next
...
opposite of "stay polyrepo" (internal-infra CONVENTIONS.md #8: shared *libraries* are imported
and versioned from a dedicated lib repo; this repo has no such dependency on internal-infra
...
internal-monitor-service's and georgia-power-monitor's deploy.sh run directly ON the target host
(desktop.example.internal/vmhost.example.internal), which does not have internal-infra checked out at all (verified
...
This keeps the two properties that make internal-infra's gate work:
...
def dirty_paths(repo_root: str, paths: list[str]) -> list[str] | None:
    ...
    Mirrors internal-infra's tools/config-drift/drift_audit.py:component_dirty_paths() --
